...
{
    /// <summary>
    /// Applies PlayMaker FSM source translations declared in translate_fsm.txt.
    /// </summary>
    public class FsmTextHook
    {
        ...
        public void Initialize(Dictionary<string, string> translations, string[] fsmTranslationFiles)
        {
            ...
            BuildTargets(fsmTranslationFiles);
            ResetRuntimeState();
        }
        ...
        public bool UpdateForScene(string currentScene, bool force)
        {
            ...
            if (changed)
                CoreConsole.Print("[FsmTextHook] Applied pending FSM translations from translate_fsm.txt in " + currentScene);

            return changed;
        }

        private void BuildTargets(string[] fsmTranslationFiles)
        {
            targets.Clear();
            ...
            if (fsmTranslationFiles == null)
                return;

            for (int i = 0; i < fsmTranslationFiles.Length; i++)
            {
                LoadTargetFile(fsmTranslationFiles[i], byKey);
            }
        ...
        private void LoadTargetFile(string filePath, Dictionary<string, FsmTarget> byKey)
        {
            ...
            try
            {
                ...
                {
                    LoadTargetLine(lines[i], filePath, i + 1, byKey);
                }
            }
            catch (System.Exception ex)
            {
                CoreConsole.Warning("[FsmTextHook] Failed to load " + Path.GetFileName(filePath) + ": " + ex.Message);
            }
        }

        private void LoadTargetLine(string line, string filePath, int lineNumber, Dictionary<string, FsmTarget> byKey)
        {
            ...
            string right = line.Substring(separatorIndex + 1).Replace("\\=", "=");
            if (line.Length > separatorIndex + 1 && line[separatorIndex + 1] == ' ')
            {
                ...
                if (!hasSecondSpace && right.Length > 0 && right[0] == ' ')
                    right = right.Substring(1);
            }

            right = TranslationFileParser.UnescapeValue(right);

            string[] parts = left.Split('|');
            if (parts.Length < 5)
            {
                CoreConsole.Warning("[FsmTextHook] Invalid FSM rule in " + Path.GetFileName(filePath) + ":" + lineNumber);
                return;
            }

            string objectPath = TranslationFileParser.UnescapeValue(parts[0].Trim());
            string fsmName = TranslationFileParser.UnescapeValue(parts[1].Trim());
            string stateName = TranslationFileParser.UnescapeValue(parts[2].Trim());
            string actionIndexText = parts[3].Trim();

            int actionIndex;
            if (!int.TryParse(actionIndexText, out actionIndex))
                actionIndex = -1;

            string source = string.Join("|", parts, 4, parts.Length - 4).Trim();
            source = TranslationFileParser.UnescapeValue(source);

            if (string.IsNullOrEmpty(objectPath) || string.IsNullOrEmpty(source))
                return;
            ...
            target.Rules.Add(new FsmRule(objectPath, fsmName, stateName, actionIndex, source, right));
        }
